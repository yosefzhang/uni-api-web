...
| 组件 | 版本 | 来源 |
| --- | --- | --- |
| uni-api（后端） | `1.7.276` | `VERSION` |
| uni-api-web（前端） | `0.5.0` | `webui/package.json` |

> 版本号会随发布更新，本地部署时可运行 `./deploy.sh status` 查看当前前后端版本。
>
> ⚠️ 上游自 `1.7.276` 起转为**纯 Rust 运行时**（移除 Python 实现）。产品版本号统一放仓库根
> `VERSION`（1.7.x，与上游 tag 对齐，Rust 侧经 build.rs 注入 `UNI_API_VERSION`）；
> `rust/uni-api-native/Cargo.toml` 里的 crate 版本是上游自己的 0.1.x，与产品版本无关。
> 前端版本号仍由 `webui/package.json` 决定镜像 Tag。
